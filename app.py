# app.py — needs-aware, turn-aware draft helper with VORP, risk, tiers & LLM
...
.callout {{
  ...
  padding:.75rem 1rem; margin:.5rem 0 1rem 0; color:#4c1d95;
}}
...
with st.expander("What this tool considers (tap to expand)", expanded=True):
    st.markdown(
        ...
        "- **Projections (optional upload):** if you upload a CSV (`name,pos,proj_pg`), we use those; "
        "otherwise we fallback to **last season full-PPR points per game** via `nfl_data_py` as a light proxy.\n"
        ...
        "- **Turn-aware:** accounts for how many picks occur before your next turn so you don’t get boxed out.\n"
...
with st.expander("Setup (league, projections, strategy)", expanded=True):
    ...
    st.markdown("**Projections (optional)** — upload CSV with columns: `name,pos,proj_pg` "
                "(per-game PPR). If omitted, we’ll use last season PPR/PG.")
...
if proj_file is not None:
    try:
        user_proj = pd.read_csv(proj_file)
        for c in ["name","pos","proj_pg"]:
            if c not in user_proj.columns:
                st.error("Projections CSV must include: name,pos,proj_pg"); st.stop()
        user_proj["pos"] = user_proj["pos"].astype(str).str.upper().str.strip()
        projections = user_proj[["name","pos","proj_pg"] + (["team","bye"] if "team" in user_proj.columns else [])]
    except Exception as e:
        st.error(f"Could not read projections CSV: {e}"); st.stop()
else:
...
board_vorp, repl_map = compute_vorp(board, int(TEAMS), NEEDS_TEMPLATE, FLEX_SHARE)
...
def llm_suggest(available_df: pd.DataFrame,
    ...
    try:
        ...
        payload = {
            ...
            "needs": needs,
            "candidates": candidates,            # ONLY these are valid
